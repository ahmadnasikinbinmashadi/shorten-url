@extends('backend::layouts.master')

@section('title', trans('backend::app.label.master_school'))

@section('header')
<style type="text/css">
    .center-align {
        text-align: center;
    }
</style>
@endsection

@section('page-header', trans('backend::app.label.master_school'))

@section('breadcrumb')
    <ol class="breadcrumb">
        <li><a href="{{ route('admin.dashboard.index') }}"><i class="fa fa-bars"></i> Home</a></li>
        <li class="active">{!! trans('backend::app.label.master_school') !!}</li>
    </ol>
@endsection

@section('content')
@include('flash::message')
        <div class="box box-primary">
            <div class="box-header">
            </div>
            {!! Form::open(['url' => '','id' => 'model-soal-form', 'class' => 'form-horizontal','autocomplete' => 'off']) !!}
                <div class="box-body">

                    <div class="form-group{{ Form::hasError('region') }}">
                        <label class="col-md-3 control-label">Wilayah</label>
                        <div class="col-sm-5">
                            {!! Form::select('region', [], [], ['class' => 'form-control','id' => 'find-region','placeholder' => '- Pilih Wilayah -']) !!}
                            {!! Form::errorMsg('region') !!}
                        </div>
                    </div>

                </div>
                <div class="box-footer">
                    <div class="pull-right">
                        <a href="{{ route('admin.school.index') }}" class="btn btn-default">
                            {!! trans('backend::app.button.back') !!} 
                        </a>

                        <a href="#" class="btn btn-primary" id="btn-find-school">
                            <span class="fa fa-refresh"></span> Syncronize
                        </a>
                    </div>

                </div>
            {!! Form::close() !!}
        </div>


    <div class="box box-primary">
         <div class="box-body">
            <table id="school-local-table"" class="table table-hover table-bordered table-condensed table-responsive" data-tables="true">
                <thead>
                    <th class="center-align">No</th>
                    <th class="center-align">Nama Sekolah</th>
                    <th class="center-align">Status Sekolah</th>
                    <th class="center-align">Alamat Sekolah</th>
                    <th class="center-align">Provinsi</th>
                    <th class="center-align">Kota</th>
                    <th class="center-align">Kecamatan</th>
                    <th class="center-align">Status</th>
                </thead>
            </table>
        </div>
    </div>
@endsection

@section('scripts')
    @include('backend::admin.school.scripts.script');
@endsection